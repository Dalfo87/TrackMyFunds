 ...
 * Con supporto per saldi negativi e conversioni da crypto a stablecoin
 */
async function recalculatePortfolio(user: string) {
  try {
    ...
    const transactions = await Transaction.find({ 
      user,
      isAutoGenerated: { $ne: true } // Escludiamo le transazioni generate automaticamente
    }).sort({ date: 1 });
    ...
    // Lista di transazioni aggiuntive generate automaticamente per stablecoin
    ...
    for (const tx of transactions) {
      ...
      if (tx.type === TransactionType.BUY || tx.type === TransactionType.AIRDROP || tx.type === TransactionType.FARMING) {
        // Gestione dell'acquisto, airdrop o farming
        if (assetIndex === -1) {
          // Se l'asset non esiste nel portafoglio, aggiungilo
          portfolio.assets.push({
            cryptoSymbol: normalizedSymbol, // Usa il simbolo normalizzato
            quantity: tx.quantity,
            averagePrice: (tx.type === TransactionType.AIRDROP || tx.type === TransactionType.FARMING) ? 0 : tx.pricePerUnit,
            category: tx.category,
            type: (tx.type === TransactionType.AIRDROP) ? 'airdrop' : 
                  (tx.type === TransactionType.FARMING) ? 'farming' : 'crypto'
          });
          console.log(`Aggiunto nuovo asset: ${normalizedSymbol}, Quantità=${tx.quantity}`);
        } else {
          // Se l'asset esiste, aggiorna quantità e prezzo medio
          const asset = portfolio.assets[assetIndex];
          
          if (tx.type === TransactionType.AIRDROP || tx.type === TransactionType.FARMING) {
            // Per gli airdrop e farming, aumentiamo la quantità ma calcoliamo il nuovo prezzo medio
            if (asset.quantity === 0) {
            ...
            console.log(`${tx.type} aggiornato: ${normalizedSymbol}, Nuova quantità=${asset.quantity}`);
          } else {
            ...
            console.log(`Acquisto aggiornato: ${normalizedSymbol}, Nuova quantità=${asset.quantity}, Nuovo prezzo medio=${asset.averagePrice}`);
          }
          
          if (tx.category) asset.category = tx.category;
        }
      } else if (tx.type === TransactionType.SELL) {
        // Gestione della vendita - CONSENTE SALDI NEGATIVI
        if (assetIndex !== -1) {
          ...
          // Riduci la quantità indipendentemente dal saldo attuale
          asset.quantity -= tx.quantity;
          console.log(`Vendita completata: ${normalizedSymbol}, Nuova quantità=${asset.quantity}`);

          // Se la quantità diventa negativa o zero, manteniamo lo stesso prezzo medio
          // Se la quantità rimane positiva, il prezzo medio non cambia durante la vendita
          console.log(`Vendita completata: ${normalizedSymbol}, Nuova quantità=${asset.quantity}, Prezzo medio invariato=${asset.averagePrice}`);
          
          // Non modifichiamo il prezzo medio durante le vendite, a meno che la quantità 
          // non passi da positiva a negativa, nel qual caso il prezzo medio è il prezzo di vendita
        if (oldQuantity > 0 && asset.quantity < 0) {
          ...
          // Manteniamo l'asset anche se la quantità è negativa
        } else {
          ...
          console.log(`Creato nuovo asset con quantità negativa: ${normalizedSymbol}, Quantità=-${tx.quantity}, Prezzo medio=${tx.pricePerUnit}`);
        }
    ...
    // Aggiungi le transazioni generate automaticamente alla lista per il calcolo
    if (additionalTransactions.length > 0) {
      console.log(`Aggiunte ${additionalTransactions.length} transazioni generate automaticamente`);
      // Nota: non è necessario elaborare queste transazioni qui perché vengono già gestite
      // direttamente nel ciclo principale (abbiamo già aggiornato gli asset)
    }
    ...
    portfolio.assets = portfolio.assets.filter(asset => {
      // Arrotonda per evitare problemi con numeri a virgola mobile (0.000000001 ecc.)
      const roundedQuantity = parseFloat(asset.quantity.toFixed(8));
      
      // Mantieni asset con quantità diversa da zero (inclusi quelli negativi)
      const keepAsset = roundedQuantity !== 0;
      
      if (!keepAsset) {
        console.log(`Rimozione asset con quantità zero: ${asset.cryptoSymbol}`);
      }
...
const recordFarming: RequestHandler = async (req, res) => {
  try {
    ...
    // Crea una nuova transazione di tipo FARMING
    const newFarming = new Transaction({
      user: 'default_user',
      cryptoSymbol: cryptoSymbol.toUpperCase(),
      type: TransactionType.FARMING,
      quantity,
      pricePerUnit: 0, // Prezzo sempre a zero per il farming
      totalAmount: 0,  // Importo totale sempre a zero per il farming
      date: date || new Date(),
      notes,
      category
    });
